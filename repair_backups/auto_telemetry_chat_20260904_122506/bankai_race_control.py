...
# =============================================================================
# SOUL FORGE — LIVE ROUTING TELEMETRY
# =============================================================================
...
def _sf_load_routing_telemetry():
    """Load the latest application-owned routing telemetry."""

    telemetry_path = (
        Path("/content/BANKAI-RACE-CONTROL")
        / "data"
...
def _sf_render_routing_telemetry(
    telemetry=None,
    stage="—",
    intent=None,
):
    """
    Render the live SOUL FORGE routing telemetry.

    This function is UI-only.
    It does not perform model routing.
    """

    if telemetry is None:
        telemetry = _sf_load_routing_telemetry()

    if not telemetry:
        st.info(
            "No routing telemetry available yet. "
            "Send a request to SOUL FORGE first."
        )
        return

    telemetry_task = _sf_telemetry_value(
        telemetry,
        "task",
        "CHAT",
    )
    ...
    if intent:
        telemetry_task = intent

    provider = _sf_telemetry_value(
        telemetry,
        "provider",
        "openrouter",
    )

    tier = _sf_telemetry_value(
        telemetry,
        "tier",
        "—",
    )

    requested_model = _sf_telemetry_value(
        telemetry,
        "requested_model",
        "—",
    )

    actual_model = _sf_telemetry_value(
        telemetry,
        "actual_model",
        "—",
    )

    latency = _sf_telemetry_value(
        telemetry,
        "latency_seconds",
        "—",
    )

    status = _sf_telemetry_value(
        telemetry,
        "status",
        "—",
    )
    ...
    requested_display = model_name(
        requested_model
    )

    actual_display = model_name(
        actual_model
    )
    ...
    st.markdown(
        ...
        <div class="sf-telemetry">
            ...
            <div class="sf-telemetry-grid">
                ...
                <div class="sf-telemetry-item {route_class}">
                    <div class="sf-telemetry-label">
                        Requested Model
                    </div>
                    <div class="sf-telemetry-value">
                ...
                <div class="sf-telemetry-item {status_class}">
                    <div class="sf-telemetry-label">
                        Provider Status
                    </div>
                    <div class="sf-telemetry-value">
