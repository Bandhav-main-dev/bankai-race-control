...
def _sf_save_tasks(tasks):
    ...
    The deployed Streamlit source directory may be read-only
    (for example /mount/src/... on Streamlit Cloud).

    We therefore:
      1. Keep the configured task path when writable.
      2. Fall back to a writable runtime directory when necessary.
      3. Keep the current task data in session state.
      4. Never allow a filesystem PermissionError to crash Task Manager.
    """
    import json
    import os
    from pathlib import Path

    # Always keep the current task state available for this session.
    try:
    ...
    candidates = [
        ...
        Path(os.environ.get("TMPDIR", "/tmp")) / "soul_forge" / "data" / "tasks.json",
    ]
    ...
    for task_path in unique_candidates:
        try:
            ...
            # Verify the directory is actually writable before writing.
            ...
            # Atomic-ish write: write temporary file, then replace target.
            temp_path = task_path.with_suffix(task_path.suffix + ".tmp")

            temp_path.write_text(payload, encoding="utf-8")
            temp_path.replace(task_path)

            # Remember where this session successfully persisted tasks.
            try:
                st.session_state["sf_task_storage_path"] = str(task_path)
    ...
    # Persistence failed everywhere, but Task Manager should continue working.
    try:
