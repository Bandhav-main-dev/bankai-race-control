...
def get_timer_display_seconds() -> int:
    """Return the current remaining seconds.

    Recalculate from the session start timestamp whenever the timer
    is actively running. This keeps the timer accurate across
    Streamlit reruns and page changes.
    """
    ensure_timer_state()

    if st.session_state.get("sf_focus_active", False):
        try:
            from app.core.timer.pomodoro_engine import _sf_focus_recalculate

            _sf_focus_recalculate()
        except Exception:
            # Never let timer display calculation crash the entire UI.
            pass

    value = st.session_state.get("sf_focus_remaining_seconds", 0)
